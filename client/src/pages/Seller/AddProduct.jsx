...
const AddProduct = () => {
    ...
    return (
        <div className="no-scrollbar flex-1 h-[95vh] overflow-y-scroll flex flex-col justify-between">
            <form onsubhandle={onsubhandle} className="md:p-10 p-4 space-y-5 max-w-lg">
                <div>
                    ...
                    <div className="flex flex-wrap items-center gap-3 mt-2">
                        {Array(4).fill('').map((_, index) => (
                            <label key={index} htmlFor={`image${index}`}>
                                ...
                                <img src={files[index] ? URL.createObjectURL(files[index]) : assets.upload_area} alt="uploadarea" className='max-w-24 cursor-pointer' width={100} height={100} />
                            </label>
                        ))}
                    </div>
                </div>
